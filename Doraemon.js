...
function generateBezierCurve(controlPoints, numPoints) {
    var t, points = [];

    for (var i = 0; i <= numPoints; i++) {
        t = i / numPoints;
        points.push(bezier(t, controlPoints));
    }

    return points;
}

// function generateSimpleSmileCurve(x, y, z, a, b, c, r, g, b) {
//   var vertices = [];
//   var colors = [];
//   var faces = [];
...
//       var xCoord = i + x;
//       var yCoord = a * Math.pow(i - b, 2) + c + y; // quadratic function for a smile
//       var zCoord = z;
...
//   var segments = vertices.length / 3;
//   for (var i = 0; i < segments - 1; i++) {
//       faces.push(i, i + 1, segments - i - 1);
//       faces.push(i + 1, segments - i - 1, segments - i - 2);
//   }
...
function generateSimpleSmileCurve(x, y, z, a, b, c, r, g, b) {
  ...
  for (var i = -1; i <= 1; i += 0.02) {
      var xCoord = i + x;
      var yCoord = a * Math.pow(i - b, 2) + c + y; // quadratic function for a smile
      var zCoord = a * Math.pow(i - b, 2) + c + z; // make z-coordinate depend on x
  ...
  function generateOblateSpheroid(x, y, z, c1, c2, c3, radius, segments, oblateness) {
    var vertices = [];
    var colors = [];
...
function generateSmileCurve(x, y, z, a, b, c, c1, c2, c3, rotationX, rotationY, rotationZ) {
  var vertices = [];
  var colors = [];

  for (var i = -1; i <= 1; i += 0.02) {
      var xCoord = i;
      var yCoord = a * Math.pow(i - b, 2) + c; // quadratic function for a smile
      var zCoord = 0;
...
// function generateReversedSmileCurve(x, y, z, a, b, c, c1, c2, c3, rotationX, rotationY, rotationZ) {
//   var vertices = [];
//   var colors = [];
...
function generateReversedSmileCurve(x, y, z, a, b, c, c1, c2, c3, rotationX, rotationY, rotationZ) {
  ...
  for (var i = -1; i <= 1; i += 0.02) {
      var xCoord = i + x; // Apply translation before rotation
      var yCoord = a * Math.pow(i - b, 2) + c + y; // Apply translation before rotation
      var zCoord = 0 + z; // Apply translation before rotation
